...
public class AreaSettingDialog extends Dialog {
    ...
    //定义父列表项List数据集合
    private List<LiveAreaListEntity> group = new ArrayList<>();
    //定义子列表项List数据集合
    private MyExpandableListAdapter myExpandableListAdapter;

    public AreaSettingDialog(@NonNull Context context, @StyleRes int themeResId) {
    ...
    public void updateData(List<LiveAreaListEntity> data) {
        group = data;
        if (!group.isEmpty()) {
            List<LiveAreaListEntity.ListBean> listBeans = group.get(0).getList();
